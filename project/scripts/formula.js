    ...
    function parseR(formula, index, level) {
      ...
      while (index < formula.length) {
        ...
        if (openBrackets.includes(ch)) {
          ...
          if (level === 0) {
            throw new FormulaError(`invalid formula; unmatched close ${ch}`, formula, index);
          }
          level--;
          index++;
          break;
      ...
      if (level > 0 && level >= startLevel) {
        throw new FormulaError("invalid formula; unmatched open bracket", formula, startIndex - 1);
      }
  
      return [elemDict, index];
    }
  
    // Handle outermost multiplier
    const [outerQuant, startIndex] = parseQuant(formula, 0);
    const [elemDict, _] = parseR(formula, startIndex, 0);
  
    // Multiply all quantities by the outermost multiplier
    for (const symbol in elemDict) {
      elemDict[symbol] *= outerQuant;
    }
